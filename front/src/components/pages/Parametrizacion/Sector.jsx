...
const Sector = () => {
  ...
  const selectPagina = (e) => {
    ...
    fetch(
      "http://127.0.0.1:3900/api/sector/listar?page=" +
        pagina +
        "&size=" +
        selectedValue
    )
  ...
  const handleSearch = (event) => {
    ...
    if (searchText.length != 0) {
      ...
      fetch(
        `http://127.0.0.1:3900/api/sector/listarEscrito?Nombre=${searchText}`
      )
        .then((response) => {
          return response.json();
        })
        .then((doc) => {
          console.log(doc);
          setSectores(doc.resultado);
        });
    } else {
      // Si el texto está vacío, vuelve a cargar los datos originales
      fetch(`http://127.0.0.1:3900/api/sector/listar?page=0&size=5`)
        .then((response) => {
          return response.json();
        })
        .then((doc) => {
