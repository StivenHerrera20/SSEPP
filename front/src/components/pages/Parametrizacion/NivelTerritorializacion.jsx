...
const NivelTerritorializacion = () => {
  ...
  const selectPagina = (e) => {
    ...
    fetch(
      "http://127.0.0.1:3900/api/nivelDeTerritorializacion/listar?page=" +
        pagina +
        "&size=" +
        selectedValue
    )
  ...
  const handleSearch = (event) => {
    ...
    if (searchText.length != 0) {
      ...
      fetch(
        `http://127.0.0.1:3900/api/nivelDeTerritorializacion/listarEscrito?Nombre=${searchText}`
      )
        .then((response) => {
          return response.json();
        })
        .then((doc) => {
          setNivel(doc.resultado);
        });
    } else {
