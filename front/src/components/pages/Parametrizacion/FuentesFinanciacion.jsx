...
const FuentesFinanciacion = () => {
  ...
  useEffect(() => {
    fetch(
      `http://127.0.0.1:3900/api/fuentesDeFinanciacion/listar?page=${pagina}&size=${fila}`
    )
      .then((response) => {
        return response.json();
      })
      .then((doc) => {
        setFuentesDeFinanciacion(doc.desarrollo);
  ...
  const selectPagina = (e) => {
    ...
    fetch(
      "http://127.0.0.1:3900/api/fuentesDeFinanciacion/listar?page=" +
        pagina +
        "&size=" +
        selectedValue
    )
  ...
  const handleSearch = (event) => {
    ...
    if (searchText.length != 0) {
      ...
      fetch(
        `http://127.0.0.1:3900/api/fuentesDeFinanciacion/listarEscrito?Nombre=${searchText}`
      )
        .then((response) => {
          return response.json();
        })
        .then((doc) => {
          console.log(doc);
          setFuentesDeFinanciacion(doc.resultado);
        });
    } else {
          ...
          <form
            ...
            onSubmit={(e) => {
              ...
              if (
                ...
                fetch(
                  `http://127.0.0.1:3900/api/fuentesDeFinanciacion/editar/${idEditar.value}`,
                  {
                    method: "PUT",
                    headers: {
                      "Content-Type": "application/x-www-form-urlencoded",
