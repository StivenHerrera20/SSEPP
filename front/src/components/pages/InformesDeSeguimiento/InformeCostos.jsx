...
const InformeCostos = () => {
  ...
  const traerData = () => {
    ...
    fetch(`http://127.0.0.1:3900/api/productoHasCosto/listarCosto/${idObjEsp}`)
      .then((response) => response.json())
      .then((data) => {
        const sumaEstimados = data.resultado.reduce(
          (total, item) => total + item.estimado,
          0
        );

        const sumaDisponibles = data.resultado.reduce(
          (total, item) => total + item.disponible,
          0
        );
        setEstimados(sumaEstimados);
        setDisponibles(sumaDisponibles);
      })
      .catch((error) => {
  ...
  const generarPDF = () => {
    if (estimados && disponible && resultado && producto) {
      const doc = new jsPDF();
      ...
      // Titulo
      doc.setFontSize(25);
      doc.text("Informe De Costos De La Política", 60, 30);
      ...
      autoTable(doc, {
        ...
        body: [
          ...
          ["Costo estimado total:", `${estimados}`],
          ["Recurso disponible:", `${disponible}`],
