...
const { EnfoqueNivelTres, sequelize } = require("../../model/Conexion");
...
router.get("/listar", async (req, res) => {
  ...
  const { count, rows } = await EnfoqueNivelTres.findAndCountAll(options);
...
router.get("/listarEscrito", async (req, res) => {
  ...
  const busqueda = await sequelize.query(
    "select * from enfoque_nivel_tres where Nombre like '%" +
      req.query.Nombre +
      "%'",
