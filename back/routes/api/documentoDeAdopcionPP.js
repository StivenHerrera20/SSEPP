...
const { documentoDeAdopcionPP, sequelize } = require("../../model/Conexion");
...
router.post("/agregar", upload.single("documento"), async (req, res) => {
  try {
    ...
    const plan = await documentoDeAdopcionPP.create({
      ...req.body,
      documento: imagePath,
    });
    res.json({
...
router.get("/listar", async (req, res) => {
  ...
  const { count, rows } = await documentoDeAdopcionPP.findAndCountAll(options);
...
router.get("/listarEscrito", async (req, res) => {
  ...
  const busqueda = await sequelize.query(
    "select * from documentos_de_adopcion_politica_publica where nombre like '%" +
      req.query.Nombre +
      "%'",
    {
...
router.delete("/eliminar/:id", async (req, res) => {
  await documentoDeAdopcionPP.destroy({
